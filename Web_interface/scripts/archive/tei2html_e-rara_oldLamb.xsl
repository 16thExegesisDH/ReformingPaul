...
<!-- setup for the footer -->
    ...
    <!-- special case: ab[@type='DropCapitalZone'] -->
    ...
    <!-- special case: ab[@type='MainZone-Head' and not(hi/choice/orig[contains(text(), 'CAP')])] -->
    <xsl:template match="ab[@type='MainZone-Head'][not(hi/choice/orig[contains(text(), 'CAP')])]">
        <span class="ab_mzHead">
            <xsl:apply-templates/>
        </span>
    </xsl:template>
    
    <!-- default case for all other ab -->
    ...
    <!--  pour les versets anciennes versions  -->
